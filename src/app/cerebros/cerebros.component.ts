...
export class CerebrosComponent implements OnInit {
  ...
  guardarDatos(id: string, flavor: string, iq: number, text: string, image: string) {
    console.log(id, flavor, iq, text, image);
    CerebrosComponent.id = id;
    CerebrosComponent.sabor = flavor;
    CerebrosComponent.iq = iq;
    CerebrosComponent.descripcion = text;
    CerebrosComponent.imagen = image;
